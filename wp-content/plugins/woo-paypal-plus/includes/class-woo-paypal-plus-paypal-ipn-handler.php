 ...
 */
class Woo_Paypal_Plus_Paypal_IPN_Handler {
    ...
    public function __construct($sandbox = false) {
        add_action('woocommerce_api_woo_paypal_plus_gateway', array($this, 'check_response'));
        add_action('valid_paypal_plus_ipn_request', array($this, 'valid_response'));
    ...
    public function check_response() {
        if(!empty($_POST) && !empty($_POST['ipn_track_id'])) {
            if (!empty($_POST) && $this->validate_ipn()) {
                ...
                do_action('valid_paypal_plus_ipn_request', $posted);
                exit;
            }
    ...
    public function valid_response($posted) {
        if (!empty($posted['custom']) && ( $order = $this->get_paypal_order($posted['custom']) )) {
            ...
            $this->add_log->add( 'paypal_plus_ipn', 'Found order #' . version_compare( WC_VERSION, '3.0', '<' ) ? $order->id : $order->get_id());
            $this->add_log->add( 'paypal_plus_ipn', 'Payment status: ' . $posted['payment_status']);
    ...
    public function validate_ipn() {
        $this->add_log->add( 'paypal_plus_ipn', 'Checking IPN response is valid');
        ...
        $this->add_log->add( 'paypal_plus_ipn', 'IPN Request: ' . print_r($params, true));
        $this->add_log->add( 'paypal_plus_ipn', 'IPN Response: ' . print_r($response, true));
        if (!is_wp_error($response) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 && strstr($response['body'], 'VERIFIED')) {
            $this->add_log->add( 'paypal_plus_ipn', 'Received valid response from PayPal');
            return true;
        }
        $this->add_log->add( 'paypal_plus_ipn', 'Received invalid response from PayPal');
        if (is_wp_error($response)) {
            $this->add_log->add( 'paypal_plus_ipn', 'Error response: ' . $response->get_error_message());
        }
        return false;
    }
    ...
    public function validate_transaction_type($txn_type) {
        ...
        if (!in_array(strtolower($txn_type), $accepted_types)) {
            $this->add_log->add( 'paypal_plus_ipn', 'Aborting, Invalid type:' . $txn_type);
            exit;
        }
    }
    ...
    public function validate_currency($order, $currency) {
        if ($order->get_order_currency() != $currency) {
            $this->add_log->add( 'paypal_plus_ipn', 'Payment error: Currencies do not match (sent "' . $order->get_order_currency() . '" | returned "' . $currency . '")');
    ...
    public function validate_amount($order, $amount) {
        if (number_format($order->get_total(), 2, '.', '') != number_format($amount, 2, '.', '')) {
            $this->add_log->add( 'paypal_plus_ipn', 'Payment error: Amounts do not match (gross ' . $amount . ')');
    ...
    public function payment_status_completed($order, $posted) {
        ...
        if ($order->has_status('completed')) {
            $this->add_log->add( 'paypal_plus_ipn', 'Aborting, Order #' . $order_id . ' is already complete.');
            exit;
        }
    ...
    public function get_paypal_order($raw_custom) {
        if (( $custom = json_decode($raw_custom) ) && is_object($custom)) {
            ...
            $this->add_log->add( 'paypal_plus_ipn', 'Error: Order ID and key were not found in "custom".');
            return false;
        }
        ...
        $order_key_val = version_compare(WC_VERSION, '3.0', '<') ? $order->order_key : $order->get_order_key();
        if (!$order || $order_key_val !== $order_key) {
            $this->add_log->add( 'paypal_plus_ipn', 'Error: Order Keys do not match.');
            return false;
        }
        return $order;
    }
