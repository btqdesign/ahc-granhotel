...
?>
<?php do_action('angelleye_paypal_plus_pay_by_invoice_instructions', $order); ?>
			...
			?>
				<tr class="data" nobr="true">
					...
					<td width="<?php echo $column_width; ?>">
						...
						<?php echo wc_price( $order->get_item_subtotal( $item, false, true ), array( 'currency' => $invoice->currency ) ); ?>
					...
					<td class="last" width="<?php echo $column_width; ?>">
						<?php echo $order->get_formatted_line_subtotal( $item, 'excl' ); ?> 
		...
		<td>
			<table class="main">
				...
				<?php if ( $order_totals = $invoice->get_totals( 'excl' ) ) : $i = 0; ?>
					...
					<?php endforeach; ?>
				<?php endif; ?>
                              <?php do_action('angelleye_display_ratenkauf_order_line_item', $order); ?>                  
			</table>
		</td>
	</tr>
</table>
